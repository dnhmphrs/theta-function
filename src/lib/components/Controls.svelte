<script>
	...
	// The four classical Jacobi thetas are the four half-integer characteristics.
	const presets = [
		{ name: 'θ₁', a: 0.5, b: 0.5 },
		{ name: 'θ₂', a: 0.5, b: 0.0 },
		{ name: 'θ₃', a: 0.0, b: 0.0 },
	...
	function applyPreset(p) {
		thetaParams.update((s) => ({ ...s, a: p.a, b: p.b }));
	}

	function isActive(p, s) {
		return Math.abs(s.a - p.a) < 1e-3 && Math.abs(s.b - p.b) < 1e-3;
	}
...
<div class="ui panel">
	...
	<div class="presets">
		{#each presets as p}
			<button class:active={isActive(p, $thetaParams)} on:click={() => applyPreset(p)}>
				{p.name}
			</button>
		{/each}
	</div>
	...
	<label class="toggle">
		<input type="checkbox" checked={$thetaParams.mode > 0.5}
			on:change={(e) => thetaParams.update((s) => ({ ...s, mode: e.target.checked ? 1 : 0 }))} />
		contours
	</label>
</div>
